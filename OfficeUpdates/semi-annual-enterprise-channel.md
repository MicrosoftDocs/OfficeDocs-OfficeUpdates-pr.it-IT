...
## <a name="version-2002-february-09"></a>Versione 2002: 9 febbraio
*Versione 2002 (build 12527.21594)*
...
## <a name="version-1908-february-09"></a>Versione 1908: 9 febbraio
*Versione 1908 (Build 11929.21008)*
...
- È stato risolto un problema che causava la chiusura imprevista dell'applicazione durante l'uso della finestra Zoom.


- Questa correzione risolve un problema per cui il tentativo di eseguire determinate query generava il messaggio di errore 'La query è troppo complessa'.


- Questa modifica risolve un problema che poteva causare la chiusura imprevista di Access durante l'avvio della casella di Zoom (MAIUSC + F2) per la modifica del testo.
...
- È stato risolto un problema per cui ora è possibile usare il tipo di dato data/ora esteso nel codice senza subire alcun arresto anomalo dell'app.
...
- È stato risolto un problema che può verificarsi quando si cerca di creare una connessione dati, se l’utente si è disconnesso dal proprio account.


- È stato corretto un bug relativo alle API PivotDateFilter, per cui le condizioni di filtro "prima" e "dopo" erano invertite.


- È stato risolto un problema per cui Excel poteva chiudersi inaspettatamente nel tentativo di inserire tabelle pivot in un foglio grafico.
...
- È stato risolto un problema per cui i clienti ricevono una notifica errata su una nuova versione del file durante la creazione condivisa.
...
- È stato risolto un problema che causava l'arresto anomalo di Excel in determinate circostanze quando si usa Copia formato.
...
- È stato risolto un problema per cui Excel poteva arrestarsi in modo anomalo quando si usa la funzionalità Analisi rapida dopo aver bloccato la riga superiore del foglio.
...
- È stato risolto un problema che in alcuni casi causava la chiusura imprevista di Excel dopo aver copiato un foglio di calcolo contenente una tabella pivot.


- È stato risolto un problema relativo al mancato funzionamento del collegamento esterno in seguito alla riapertura del file se il percorso del file è troppo lungo.


- È stato risolto un problema legato all'uso dei dati in tempo reale in combinazione con la funzionalità di ricalcolo multithread, che causava la chiusura imprevista dell'applicazione.
...
- È stato risolto un problema per cui il menu del controllo ortografico non veniva caricato.


### <a name="outlook"></a>Outlook

- È stato risolto un problema per cui le esperienze connesse facoltative bloccavano il caricamento dei componenti web aggiuntivi.  Altri dettagli:  https://developer.microsoft.com/en-us/office/blogs/outlook-add-ins-and-optional-connected-experiences/
...
- Risolve un problema che faceva sì che gli utenti sperimentassero occasionalmente delle chiusure impreviste durante l’interazione con gli allegati cloud.


- È stato risolto un problema che causava la chiusura anomala di CLP cambiando l'indirizzo Da in una risposta da un contesto protetto a uno non protetto.
...
- È stato risolto un problema che causava problemi per gli utenti quando si cancellano 4 o più messaggi di posta elettronica da un account POP quando l'opzione "Scarica solo intestazioni" è selezionata.
...
- È stato risolto un problema che causava un problema nelle applicazioni MAPI di terza parte.


- È stato risolto un problema che causava la chiusura imprevista di Outlook aprendo file .msg o .oft salvati in locale dopo un aggiornamento di Windows.


- È stato risolto un problema che causava la chiusura imprevista di Outlook in alcune build di Windows.
...
- È stato risolto un problema relativo al caricamento degli allegati.
...
- È stato risolto un problema che causava un problema quando si usava il pulsante "X" del mouse.
...
- È stato risolto un problema per cui le opzioni Solo crittografa e Non inoltrare continuavano a essere abilitate in alcuni scenari, nonostante fossero disabilitate dai criteri.
...
- È stato risolto un problema che causava occasionalmente chiusure impreviste per gli utenti durante il recupero delle informazioni personali.


- Questo risolve un problema che causava occasionalmente agli utenti delle chiusure dell'applicazione durante la modifica dei destinatari.
...
- È stato risolto un problema per cui il componente aggiuntivo per i contenuti di Microsoft Forms non veniva eseguito dopo l'inserimento finché l'utente non faceva clic su un'altra diapositiva per visualizzarlo.


- Correzione di sicurezza che risolve un problema che disabilitava le protezioni IRM aprendo un file di PowerPoint nella modalità Visualizzazione protetta.
...
- È stato risolto un problema relativo al riquadro dei suggerimenti che poteva causare la chiusura imprevista dell'applicazione.


- È stato risolto un problema nella finestra di dialogo Impostazioni azione che causava la chiusura imprevista dell'app PowerPoint.


- È stato risolto un problema di PowerPoint che causava la chiusura imprevista dell'applicazione.
...
- È stato risolto un problema per cui la presentazione nella modalità di visualizzazione protetta non funzionava.
...
- È stato risolto un problema che causava l'arresto anomalo di Project quando si salvano progetti creati con versioni precedenti.
...
- È stato risolto un problema che causava la chiusura imprevista dell'anteprima in tempo reale durante l'allineamento del testo.
...
- È stato risolto un problema per cui le intestazioni con numeri incollate avevano un rientro aggiuntivo.
...
- È stato risolto un problema che causava l'arresto anomalo all'apertura dei documenti.
...
- Risolve un bug delle estensioni di commento, per cui le risposte di commento avevano la stessa estensione del commento padre.
...
- Risolve un problema che può aver causato la chiusura imprevista dell'applicazione all'esecuzione.
...
- È stato risolto un problema del vecchio pannello Condividi basato su servizi non online, che causava la chiusura imprevista dell'applicazione quando si chiudeva un documento mentre il riquadro Condividi era aperto. 


- È stato risolto un problema che poteva causare la chiusura imprevista dell'applicazione durante la chiusura dei file di Office.
...
- Questa modifica risolve un problema all'avvio delle app di Office dovuto al mancato caricamento del file d2d1.dll.


- L'host di Office si chiudeva in modo imprevisto in Windows, quando veniva attivato un componente aggiuntivo mentre la chiave del registro di sistema HKEY_CURRENT_USER \Software\Microsoft\Internet Explorer\Main\TabProcGrowth era impostata su zero. La modifica può risolvere il problema.


- Risolve un problema relativo al malfunzionamento della API di messaggistica per i componenti aggiuntivi di Office.
...
## <a name="version-2002-january-12"></a>Versione 2002: 12 gennaio
*Versione 2002 (Build 12527.21504)*
...
## <a name="version-1908-january-12"></a>Versione 1908: 12 gennaio
*Versione 1908 (Build 11929.20994)*
...
- È stato risolto un errore per cui alcune funzioni avrebbero avuto un risultato errato dopo il caricamento di una cartella di lavoro.


- È stato risolto un problema per cui l'applicazione si chiudeva in modo imprevisto, correlato ai riferimenti del componente aggiuntivo XLAM e agli intervalli denominati.


- È stato risolto un problema per cui l'esecuzione della macro del filtro avanzato riportava erroneamente errori.


### <a name="outlook"></a>Outlook

- È stato risolto un problema che causava la disabilitazione imprevista dei componenti aggiuntivi interni quando venivano disabilitate le esperienze connesse facoltative.


- È stato risolto un problema che impediva agli utenti di inviare un messaggio di posta elettronica per conto di una lista di distribuzione nascosta dall'elenco indirizzi globale.



[//]: # (NON RIMUOVERE FINE DEL CONTENUTO BUG)

## <a name="version-1908-november-10"></a>Versione 1908: 10 novembre
*Versione 1908 (Build 11929.20974)*
...
## <a name="version-2002-september-08"></a>Versione 2002: 8 settembre
*Versione 2002 (Build 12527,21104)*
...
## <a name="version-1908-september-08"></a>Versione 1908: 8 settembre
*Versione 1908 (Build 11929.20946)*
...
- **Creare PDF più accessibili:** quando si crea un file PDF, la funzione Verifica accessibilità indica i problemi di accessibilità da correggere prima di salvare. [Altre informazioni](https://support.office.com/article/064625e0-56ea-4e16-ad71-3aa33bb4b7ed)<br />Vedere i dettagli nel [post di blog](https://blog-insider.office.com/2019/08/13/accessible-pdfs-made-easier/)
...
- **Gestire al meglio le cartelle di lavoro di grandi dimensioni:** celle, formule, grafici, tabelle... ottenere un’istantanea della cartella di lavoro con l’opzione Statistiche cartella di lavoro.
...
- **Suggerimenti per il luogo delle riunioni:** è sufficiente iniziare a scrivere nella riga Luogo durante la pianificazione di appuntamenti e riunioni e Outlook suggerirà sale, indirizzi e altri luoghi recenti. [Altre informazioni](https://support.office.com/article/1d8631be-611a-4e3d-9109-b153e4622d53)<br />Vedere i dettagli nel [post di blog](https://blog-insider.office.com/2019/07/08/location-suggestions-in-outlook-for-windows/)
...
- **Creare PDF più accessibili:** quando si crea un file PDF, la funzione Verifica accessibilità indica i problemi di accessibilità da correggere prima di salvare.<br />Vedere i dettagli nel [post di blog](https://blog-insider.office.com/2019/08/13/accessible-pdfs-made-easier/)
...
- **Ottimizzazione della presentazione per tutti:** Verifica accessibilità consente di organizzare gli oggetti nelle diapositive tenendo in considerazione le utilità per la lettura dello schermo.<br />Vedere i dettagli nel [post di blog](https://blog-insider.office.com/2019/10/15/reach-more-people-by-making-your-powerpoint-slides-work-with-a-screen-reader/)
...
- **Altre icone che corrispondono al proprio umore:** sono state aggiunte più di 300 nuove icone. Per scoprirle, scegliere Inserisci > Icone. [Altre informazioni](https://support.office.com/article/3b179567-785e-42ac-8544-ec4ee5ebf1c8)<br />Vedere i dettagli nel [post di blog](https://blog-insider.office.com/2019/04/24/your-feedback-in-action-new-insertable-icons/)
...
- Aprire una cartella di lavoro con riferimenti a molte altre cartelle di lavoro, in particolare con finestre nascoste, risulta un'operazione piuttosto lenta.

- È stato risolto un problema per cui i collegamenti esterni non venivano aggiornati durante il riempimento (copia in basso o tra fogli di lavoro) se la cartella di lavoro di origine era& chiusa.

- È stato risolto un problema per cui Excel smetteva di funzionare dopo avere premuto i tasti CTRL+MAIUSC+Freccia per scorrere, quando la finestra di Excel era condivisa attraverso Teams.

- Per le cartelle di lavoro salvate con una firma digitale in Excel 2016 la firma poteva essere invalidata con l’apertura nell’attuale versione di Excel.

- Risolve un problema per cui la proprietà "Intersezione valore" sull'asse di un grafico cambia in modo imprevisto in caso di salvataggio e riapertura di un file.

- È stato risolto un problema per cui la personalizzazione CustomUI XML per la scheda della barra multifunzione veniva rimossa durante il salvataggio su SharePoint/OneDrive.
...
- È stato risolto un problema per cui una cartella di lavoro poteva venire nascosta dopo aver fatto clic su un collegamento ipertestuale a un punto in una cartella di lavoro già aperta.
...
- Questo aggiornamento risolve un problema per cui nella barra della formula veniva visualizzato del testo in un carattere diverso da quello previsto.
...
- Risolve un problema di prestazioni durante la creazione di grafici dai modelli.
...
- È stato risolto un problema per cui un simbolo @ che avvia una formula veniva considerato come operatore di intersezione implicito.
...
- Migliora la sincronizzazione e la stabilità del servizio modificando temporaneamente la frequenza di sincronizzazione in OneNote 2016.

- Migliora la sincronizzazione e la stabilità del servizio rimandando temporaneamente il download di immagini e file incorporati in blocchi appunti online finché l'utente esplora la pagina in OneNote 2016.

- Migliora la sincronizzazione e la stabilità del servizio disabilitando temporaneamente il cestino in OneNote 2016. Quando un utente prova a eliminare dati che normalmente verrebbero spostati nel cestino, all'utente verrà chiesto se vuole mantenere o eliminare definitamente il file.

- Migliora la sincronizzazione e la stabilità del servizio riducendo temporaneamente il numero e la frequenza di sincronizzazione delle pagine della cronologia versioni di OneNote 2016.
...
- Migliora la sincronizzazione e la stabilità del servizio riducendo temporaneamente a 50 MB la dimensione massima consentita per i nuovi allegati incorporati in OneNote 2016. Per i file che superano questo limite, gli utenti avranno la possibilità di caricare il file in OneDrive e inserire un collegamento in OneNote.

- Migliora la sincronizzazione e la stabilità del servizio disabilitando temporaneamente la registrazione dei video in-app in OneNote 2016. I blocchi appunti locali non vengono interessati da questa operazione.
...
- Risolve un problema per cui gli utenti visualizzavano arresti anomali occasionali in Outlook.
...
- Risolve un problema per cui appuntamenti o riunioni ricorrenti venivano visualizzati all’orario sbagliato quando era necessario cambiare la definizione del fuso orario.

- Quando si usa il fuso orario di Brasilia nell'anno 2019, le riunioni e gli appuntamenti ricorrenti vengono visualizzati nella fascia oraria sbagliata per l'anno 2020. Questa modifica è rilevante per i client configurati per il fuso orario di Brasilia o per le riunioni e gli appuntamenti impostati in tale fuso orario.<br><br>Questa modifica consente a Outlook di ignorare determinate impostazioni del fuso orario usate da Outlook. Per poter ignorare un fuso orario è necessario impostare una chiave del Registro di sistema per l'utente corrente. Il nome della chiave del Registro di sistema deve corrispondere al nome interno del fuso orario. Il valore indica l’anno della regola del fuso orario da usare al posto dell'anno effettivo. Ad esempio, il seguente valore di override della chiave del Registro di sistema userà la regola del fuso orario di Brasilia per l'anno 2020 come regola effettiva. HKEY_CURRENT_USER\Software\Microsoft\Office\16.0\Outlook\TimeZoneOverride]"E. South America Standard Time"=dword:000007e4.

- Risolve un problema che causava la modifica imprevista del campo della sede nelle riunioni.

- Risolve un problema che causava l'aggiornamento imprevisto del campo Sede nelle riunioni.

- Risolve un problema per cui la sede di una riunione veniva inaspettatamente aggiunta di nuovo alla riunione dopo averla cancellata.

- Risolve un problema per cui la virgola nel campo Sede della riunione cambiava in punto e virgola.

- Consente di partecipare a una riunione di Teams direttamente tramite il client nativo di Teams.

- Risolve un problema relativo all'aggiunta di allegati agli elementi del calendario per utenti con cassette postali sui server di Exchange 2010.

- Risolve un problema relativo alla risposta a messaggi con firma digitale.

- Risolve un problema che impediva agli utenti di aprire alcune istanze di elementi ricorrenti del calendario.

- Risolve un problema che causava l'espansione improvvisa dell'intestazione del gruppo in alcuni scenari.

- Risolve un problema che causava il cambiamento inatteso della larghezza del riquadro delle cartelle.
...
- Risolve un problema che causava un considerevole ritardo nell'interazione con le cartelle delle cassette postali degli utenti usando le scelte rapide da tastiera.

- Risolve un problema per cui si verificava un arresto anomalo quando si visualizzava lo stesso elemento in più finestre.
...
- Risolve un problema che causava una perdita di memoria in caso di sessioni di Outlook molto lunghe.

- Risolve un problema che causava un arresto anomalo specificando un indirizzo Da non valido.

- Risolve un problema che causava la visualizzazione del messaggio “Le regole impostate in questo computer non corrispondono alle regole impostate in Microsoft Exchange” all'apertura della finestra di dialogo Regole.

- Risolve un problema per cui, in alcuni scenari, l'opzione per disabilitare l'evidenziazione degli elementi contrassegnati non veniva rispettata.

- Risolve un problema che causava l'invio imprevisto di messaggi di posta elettronica quando gli utenti premevano il tasto “S” dopo aver chiuso il riquadro Verifica accessibilità.

- Risolve un problema che causava arresti anomali quando gli utenti rinominavano una firma.

- Risolve un problema che causava un arresto anomalo annullando la configurazione dell'account.

- Risolve un problema per cui Outlook sincronizzava in modo imprevisto tutta la posta elettronica anche quando il dispositivo di scorrimento di sincronizzazione era impostato su un valore inferiore.

- Risolve un problema per cui gli utenti con il tema Nero visualizzavano l'elenco a discesa "Da" con il testo bianco su sfondo bianco.

- Risolve un problema che causava un arresto anomalo durante la creazione del profilo.

- Risolve un problema che causava la visualizzazione di una finestra di messaggio vuota con un pulsante “OK” quando si provava a contattare il supporto dal contesto di creazione dell'account.

- Risolve un problema che causava la visualizzazione di una finestra di messaggio vuota con un pulsante “OK” quando si provava a contattare il supporto dal contesto di creazione dell'account.

- Risolve un problema per cui le applicazioni di terze parti non riuscivano a inviare messaggi di posta elettronica.

- Risolve un problema che causava la scomparsa occasionale delle categorie dai messaggi di posta elettronica.

- Risolve un problema che restituiva agli utenti di Outlook sui sistemi operativi del server l'errore: "Stato antivirus: non valido. Questa versione di Windows supporta il rilevamento dei programmi antivirus, ma non è stato trovato alcun programma antivirus”, anche se l’antivirus è stato configurato in modo appropriato.
...
- Risolve un problema per cui i delegati ricevevano un messaggio di errore modificando un appuntamento del calendario esistente nel calendario di un responsabile.

- Risolve un problema che impediva agli utenti con l'impostazione errata dell'emulazione del browser di completare la richiesta di autenticazione di Gmail.

- Risolve un problema per cui l'opzione “Consenti inoltro” non era presente tra le opzioni di risposta in una riunione nel calendario condiviso, se la cartella di download condivisa NON era selezionata.

- Risolve un problema che causava un arresto anomalo quando si cercava di aprire file .msg e .oft dopo un aggiornamento di Windows.

- Risolve un problema che causava un arresto anomalo occasionale quando veniva usato il pulsante X del mouse.

- Risolve un problema che causava un arresto anomalo quando venivano selezionati alcuni risultati della ricerca.

- Risolve un problema che causava l'assenza del pulsante Salva nel cloud in Strumenti allegati.

- Questa modifica ripristina la possibilità di visualizzare oggetti multilinea nell'intestazione del messaggio.

- Risolve un problema che causava un arresto anomalo quando due componenti aggiuntivi aggiungevano un pulsante allo stesso gruppo della barra multifunzione.

- Risolve un problema che causava la mancata aggiunta di collegamenti ai messaggi di posta elettronica con l'autorizzazione predefinita del tenant corretta quando questa era configurata su "chiunque".

- Risolve un problema che impediva agli utenti di inviare messaggi di posta elettronica a una lista di distribuzione personale.
...
- Risolve un problema per cui il corpo di un messaggio NDR passava da Unicode a ASCII dopo aver modificato l'oggetto.

- Risolve un problema per cui gli utenti notavano una perdita di memoria nel processo di Outlook.
...
- Risolve un problema che causava l’errato allineamento della casella di ricerca in modalità dpi elevato.

- Risolve un problema che causava il blocco di Outlook durante il recupero delle impostazioni del cloud.

- Risolve un problema che causava il blocco dell'esperienza di Feedback sulla ricerca.
...
- Risolve un problema relativo all'inoltro della messaggistica corretta per gli utenti che aprono una copia di un file con commenti migliorati.
...
- Il team ha aggiunto il supporto client per la creazione di report di telemetria sui domini CDN di Office nell'anteprima aziendale semestrale.
...
- Questo aggiornamento consente di risolvere un problema per cui Microsoft Outlook non visualizza l'etichetta di riservatezza corrente durante la visualizzazione o la creazione di messaggi.
...
- L'host di Office si chiudeva in modo anomalo in Windows quando veniva attivato un componente aggiuntivo mentre la chiave del registro di sistema  HKEY_CURRENT_USER\Software\Microsoft\Internet Explorer\Main\TabProcGrowth era impostata su zero. La modifica può risolvere il problema.

- È stato risolto il problema per cui Access e Publisher potevano non venire avviati correttamente a seconda delle lingue installate.
...
## <a name="version-1908-july-14"></a>Versione 1908: 14 luglio
*Versione 1908 (Build 11929.20904)*
...
- Quando si copiavano dati filtrati in base al colore in una colonna di larghezza differente, i valori non venivano incollati.
...
- Risolve un problema che causava l'accesso indesiderato dei componenti aggiuntivi Web ai messaggi con contenuto digitale protetto.

- Risolve un problema che causava la mancata visualizzazione degli URL al passaggio del mouse per alcuni collegamenti sicuri.
...
- Risolve un problema che causava un errore di autorizzazione degli utenti durante la copia di elementi dal calendario principale a un calendario di gruppo.

- Risolve un problema che impediva agli utenti di accedere ai suggerimenti per la posizione con un'utilità per la lettura dello schermo.
...
- Risolve un problema che causava un arresto anomalo di Outlook durante l'interazione con determinati collegamenti sicuri.

- Risolve un problema che causava ambiguità per i responsabili circa il fatto che un delegato avesse già risposto o meno a una determinata convocazione di riunione.

- Risolve un problema che causava un arresto anomalo durante l'elaborazione delle risposte del servizio di individuazione automatica.
...
- Risolve un problema che causava il malfunzionamento della ricerca in Gruppi moderni.

- Risolve un problema che bloccava gli utenti di Outlook nello stato "Password necessaria" in alcuni scenari.

- Risolve un problema che causava errori di sincronizzazione durante l'elaborazione di messaggi di conflitto.

- Risolve un problema che causava un arresto anomalo durante l'apertura di file .msg e .oft dopo un aggiornamento di Windows recente.

- Risolve un problema che causava il blocco della schermata di caricamento del profilo durante l'avvio di Outlook.

- Risolve un problema che causava un arresto anomalo quando venivano selezionati alcuni risultati della ricerca.

- Risolve un problema che causava l'assenza del pulsante "Salva nel cloud" da Strumenti allegati.

- Per impostazione predefinita, le etichette dei criteri di conservazione visualizzano il periodo di conservazione tra parentesi. Questa operazione fornisce una chiave del registro di sistema che consente agli amministratori di specificare che solo il nome del criterio deve essere visualizzato. HKCU\SOFTWARE\Microsoft\Office\16.0\Outlook\Preferences\HideRetentionPolicyDuration 0 = default 1 = verrà mostrato solo il NomeCriterio nel testo dei criteri di conservazione.

- Risolve un problema che causava un arresto anomalo durante la chiusura di Outlook.

- Risolve un problema che causava la visualizzazione di un elenco sale vuoto in alcuni scenari.

- Risolve un problema che causava arresti anomali intermittenti cambiando visualizzazione.
...
- Risolve un problema che causava la mancata visualizzazione di Suggerimenti per i criteri usando un mittente alternativo.

- Risolve un problema che causava la visualizzazione delle sale consigliate per le riunioni in orari al di fuori della disponibilità della sala.
...
- Risolve un problema che causava l'arresto anomalo provando a creare una regola da un messaggio di tipo "Conversazione non effettuata".

- Risolve un problema per cui alcuni utenti visualizzavano cartelle speciali duplicate create durante l'aggiunta di un account di Exchange secondario.

- Risolve un problema che causava il troncamento del corpo del messaggio durante l'inoltro di messaggi HTML di grandi dimensioni.

- Risolve un problema per cui gli utenti notavano una perdita di memoria nel processo di Outlook.

- Risolve un problema che causava arresti anomali intermittenti quando si aggiornavano le informazioni sulla presenza.

- Risolve un problema a causa del quale non era talvolta possibile eseguire la ricerca della cartella corrente.

- Risolve un problema che causava la visualizzazione di errori di autenticazione per alcuni utenti quando provavano a recuperare le impostazioni del cloud per Outlook.
...
- È stato risolto un problema che causava la creazione di schemi duplicati quando si copiava una diapositiva da una presentazione a un'altra.<br>
...
- Impedisce un errore in PowerPoint quando si prova a presentare online.
...
- È stato risolto un problema relativo alle revisioni che a volte generavano un ciclo infinito.
...
- Risolve diversi problemi che causavano il blocco dell'app durante la chiusura. Sono stati risolti anche alcuni arresti anomali correlati ai componenti aggiuntivi.
...
- Risolve un arresto anomalo di Word abbandonando un'API deprecata.

- È in corso la risoluzione di un problema per cui i caratteri katakana ridotti non ruotavano correttamente nelle caselle di testo verticali in PowerPoint.
...
- I nomi dei canali per i fork gennaio e luglio 2019 sono stati aggiornati ai nuovi nomi di canale.

- Risolve un problema per cui gli aggiornamenti potrebbero essere stati bloccati in modo imprevisto sulle reti a consumo.
...
- Risolve un problema per cui, in alcuni casi, un aggiornamento non veniva applicato se l'utente non era un amministratore e non era disponibile la connettività di rete per l'account di sistema.
...
- Risolvere problemi relativi alla proprietà di adattamento automatico della Casella di testo/Forma nei plug-in di terze parti.
...
## <a name="version-1902-july-14"></a>Versione 1902: 14 luglio
*Versione 1902 (Build 11328.20624)*

Aggiornamenti della sicurezza elencati [qui](https://docs.microsoft.com/officeupdates/microsoft365-apps-security-updates)

## <a name="version-1908-june-09"></a>Versione 1908: 09 giugno
*Versione 1908 (Build 11929.20838)*
...
- I nomi dei canali sono stati aggiornati per i fork gennaio e luglio 2019 ai nuovi nomi di canale.


[//]: # (DO NOT REMOVE BUGDETAILS CONTENT END)

## <a name="version-1902-june-09"></a>Versione 1902: 09 giugno
*Versione 1902 (Build 11328.20602)*
...
- Sono stati rimossi riferimenti obsoleti dai file di base che stavano interrompendo la build C2R.



[//]: # (DO NOT REMOVE BUGDETAILS CONTENT END)

## <a name="version-1908-may-12"></a>Versione 1908: 12 maggio
*Versione 1908 (Build 11929.20776)*
...
- È stato risolto un problema che causava un arresto anomalo durante l'apertura di file .msg e .oft dopo un aggiornamento recente di Windows.
...
## <a name="version-1902-may-12"></a>Versione 1902: 12 maggio
*Versione 1902 (Build 11328.20586)*
...
- È stato risolto un problema che causava un arresto anomalo durante l'apertura di file .msg e .oft dopo un aggiornamento di Windows recente.

- Per impostazione predefinita, le etichette dei criteri di conservazione visualizzano il periodo di conservazione tra parentesi. Questa operazione fornisce una chiave del registro di sistema che consente agli amministratori di specificare che solo il nome del criterio deve essere visualizzato. HKCU\SOFTWARE\Microsoft\Office\16.0\Outlook\Preferences\HideRetentionPolicyDuration. 0 = Predefinito.  1 = Verrà visualizzato solo il NomeCriterio nel testo dei criteri di conservazione.
...
- È stato risolto un problema in VBA per cui scrivere valori in un intervallo era più lento del previsto.
...
- Quando si aprivano nella versione corrente di Excel le cartelle di lavoro salvate con una firma digitale in Excel 2016, la firma poteva essere invalidata.
