...
### <a name="version-1910-october-24"></a>Versione 1910: 24 ottobre
*Versione 1910 (Build 12130.20238)*
...
- <div>È stato risolto un problema di prestazioni relativo alle funzioni asincrone definite dall'utente che causava l'esecuzione sincrona di tali funzioni.</div>
...
- <div><span>È stato risolto un problema che poteva comportare una differenza tra il nome del carattere sulla barra multifunzione e il tipo di carattere usato</span></div>
...
- <h5>Scenario utente</h5><br><h5>Difetto</h5><br><h5>Correzione</h5><br><h5>Note di sviluppo/test</h5><br><h5>Revisione del codice</h5><br>
...
- <div><span>È stato risolto un problema per cui l'area di stampa in anteprima di stampa non risultava corretta</span></div>
...
- <div><span>È stato risolto un problema per cui facendo clic su un collegamento a una pagina copiata veniva visualizzata la pagina originale anziché quella appena copiata</span></div>


### <a name="opc"></a>OPC

- <div><span></span></div><span style="font-size:11.0pt;font-family:&quot;Calibri&quot;,sans-serif;">È stato risolto un problema che poteva comportare il consumo inappropriato delle risorse da parte di Outlook quando la Modalità protetta era disabilitata per i siti con restrizioni in Internet Explorer</span>
...
- <div><span>È stato risolto un problema che impediva la visualizzazione dei risultati della ricerca espansa dei pacchetti in anteprima nei risultati della ricerca</span></div>
...
- <div>È stato identificato un problema per cui i dispositivi ARC potrebbero perdere la connessione se in esecuzione in AirSpace WinComp</div>
...
- <div>È stato risolto un problema per cui<span><span style="display:inline !important;background-color:rgba(255, 255, 255, 1);">i progetti venivano salvato in Project Web App tramite un progetto principale in cui non era possibile salvare le risorse locali, le tariffe ed altre proprietà dei sottoprogetti.</span><br></span></div>
...
- <div><span>È stato risolto un problema per cui, se si dispone di un progetto principale e si incollano i valori dei campi personalizzati locali del progetto principale in un progetto inserito, alcune informazioni sull'attività non vengono salvate correttamente.</span></div>
...
- <div><span>È stato risolto un problema che poteva comportare la visualizzazione di un avviso ripetuto per l'eliminazione delle versioni locali di un file</span></div>


- <div>È stato risolto un problema per cui poteva essere proposta la cronologia delle versioni anche se la funzionalità era disabilitata</div>


- <div>Il nome di accesso viene ora prepopolato <span style="background-color:rgb(255, 255, 255);display:inline !important;">per le richieste di autenticazione di base in Outlook</span></div>
...
- **Componente aggiuntivo di Excel Visualizzatore dati di Visio:** consente di visualizzare i dati in modo semplice e rapido nei diagrammi di Visio in Excel. [Altre informazioni](https://support.office.com/it-IT/article/bee3b5aa-aaaf-4401-acc6-276b711c763c)
...
- È stato risolto un problema per cui gli utenti potevano ricevere un errore di &quot;stato incoerente&quot; durante l'uso di un database condiviso.

### <a name="excel"></a>Excel

- È stato risolto un problema per cui l'area di stampa in anteprima di stampa non risultava corretta.

- È stato risolto un problema che impediva l'aggiornamento delle tabelle pivot durante una sessione di creazione condivisa.

### <a name="outlook"></a>Outlook

- È stato risolto un problema che causava la duplicazione delle cartelle di posta.

- È stato risolto un problema che causava un messaggio di errore non corretto quando si provava a inviare posta elettronica crittografata s/MIME.

- È stato risolto un problema che comportava un arresto anomalo quando un utente riceveva un messaggio di "Conversazione non effettuata" da Skype.

- È stato risolto un problema che causava una perdita di memoria.

- È stato risolto un problema che poteva causare la modifica del nome del mittente al salvataggio del messaggio come bozza.

### <a name="powerpoint"></a>PowerPoint

- È stato risolto un problema che causava il danneggiamento dei TextRanges dopo aver incollato il testo nei segnaposto intestazione/piè di pagina/numero nello schema diapositiva e layout di diapositiva.

- Abbiamo risolto un problema che impediva la creazione di un collegamento ipertestuale quando si incollava il testo con tale collegamento.

- È stato risolto un problema che impediva il corretto funzionamento delle statistiche.

### <a name="word"></a>Word

- È stato risolto un problema per cui i colori dei tipi di carattere non venivano confermati.

### <a name="office-suite"></a>Famiglia di prodotti Office

- È stato risolto un problema che forniva la cronologia delle versioni quando tale caratteristica era disabilitata.
...
- È stato risolto un problema che poteva causare la segnalazione di errori di autorizzazione durante l'interazione con cartelle di calendari condivisi.

- È stato risolto un problema che poteva impedire agli utenti di aggiungere allegati ai calendari.

- È stato risolto un problema che causava la visualizzazione di messaggi di errore rispondendo a un messaggio con firma digitale.
...
- È stato risolto un problema per cui il testo in grassetto medio poteva essere reso con lo stile errato.

- È stato risolto un problema per cui l'utente poteva ricevere un messaggio di errore non corretto alla chiusura di un file con caricamento in sospeso.
...
- È stato risolto un problema per cui Excel poteva talvolta bloccarsi all'avvio.

### <a name="outlook"></a>Outlook

- Sono state notevolmente migliorate le prestazioni di selezione della sala quando sono disponibili numerose sale.

- È stato risolto un problema che può impedire la sincronizzazione della cassetta postale per i clienti che hanno più cassette postali di Outlook quando si esegue la migrazione all'autenticazione moderna in Office 365.

- È stato risolto un problema per cui alcuni caratteri nelle etichette delle firme non vengono visualizzati nel menu a discesa.

### <a name="project"></a>Project

- È stato risolto un problema che poteva causare un arresto anomalo sostituendo una risorsa dell'organizzazione con una risorsa locale.

### <a name="word"></a>Word

- È stato risolto un problema che poteva impedire il funzionamento corretto dello scorrimento sincrono in visualizzazione Bozza.

- È stato risolto un problema che poteva impedire la visualizzazione corretta delle descrizioni comandi dopo aver salvato un documento per la prima volta.
...
- È stato risolto un problema che impediva di incollare collegamenti ipertestuali in alcuni fogli protetti.

### <a name="outlook"></a>Outlook

- È stato risolto un problema per cui l'interfaccia utente poteva rimanere bloccata in una visualizzazione compatta.

### <a name="powerpoint"></a>PowerPoint

- È stato risolto un problema per cui poteva verificarsi un errore durante la stampa in formato PDF.

### <a name="project"></a>Project

- È stato risolto un problema per cui le modifiche apportate a un valore di lavoro per un'attività di riepilogo potevano causare un arresto anomalo se era abilitato il lavoro protetto.

- È stato risolto un problema che poteva impedire la sincronizzazione degli eventi con i calendari dell'organizzazione.

### <a name="office-suite"></a>Famiglia di prodotti Office

- È stato risolto un problema per cui poteva essere visualizzato un avviso ripetuto per l'eliminazione delle versioni locali di un file.
...
- È stato risolto un problema che poteva comportare una differenza tra il nome del carattere sulla barra multifunzione e il carattere utilizzato.

### <a name="outlook"></a>Outlook

- È stato risolto un problema che poteva comportare il consumo inappropriato delle risorse da parte di Outlook quando la Modalità protetta era disabilitata per i siti con restrizioni in Internet Explorer.

- È stato risolto un problema che a volte poteva causare la comparsa di caratteri Unicode quando si incolla il testo da un'origine ANSI.
...
- È stato risolto un problema che poteva comportare la perdita della formattazione della tabella.

- È stato risolto un problema che poteva compromettere la scelta rapida da tastiera CTRL+V.
